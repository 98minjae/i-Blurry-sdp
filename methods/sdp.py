...
class SDP(ER):
    ...
    def online_train(self, sample, batch_size, n_worker, iterations=1, stream_batch_size=0):
        ...
        for i in range(iterations):
            self.model.train()
            data = self.memory.get_batch(batch_size)
    ...
    def online_evaluate(self, test_list, sample_num, batch_size, n_worker, csv_path):
        test_df = pd.DataFrame(test_list)
        exp_test_df = test_df[test_df['klass'].isin(self.exposed_classes)]
        
        print(f"Exposed classes: {self.exposed_classes}")
        # Exposed classes: ['deer', 'ship', 'horse', 'airplane', 'automobile', 'frog']
        # Exposed classes: ['deer', 'ship', 'horse', 'airplane', 'automobile', 'frog', 'cat']
        ...
        # Save as csv (Append)
        predicted_labels = [item[0] for item in eval_dict['preds']]
        predicted_labels = [self.exposed_classes[label] for label in predicted_labels]
        
        image_path_pred_dict = {
            test_dataset[i]['image_name']: predicted_labels[i] for i in range(len(test_dataset))
        }
        
        # Update csv
        df = pd.read_csv(csv_path, index_col=0)
        if f"sample_{sample_num}" not in df.columns:
            new_column_data = {f"sample_{sample_num}": ['unseen'] * len(test_df)}
            new_df = pd.DataFrame(new_column_data)
            df = pd.concat([df, new_df], axis=1)
            df[f"sample_{sample_num}"] = df['path'].map(image_path_pred_dict).fillna(df[f"sample_{sample_num}"])
            
            # df.loc[:len(predicted_labels) - 1, f"sample_{sample_num}"] = predicted_labels
            df.to_csv(csv_path)
    ...
    def evaluation(self, test_loader, criterion):
        ...
        with torch.no_grad():
            for i, data in enumerate(test_loader):
                ...
                _, preds = logit.topk(self.topk, 1, True, True)
                total_preds.extend(np.array(preds.cpu()).tolist())
